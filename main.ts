...
enum PermalinkStyle {
	None = "none",
	NanoId = "nanoId",
	FileNameUrlEncoded = "file name (url encoded)",
	FileNameNoUrlEncoding = "file name (no url encoding)",
}
...
const DEFAULT_SETTINGS: JekyllExportSettings = {
	...
	openaiApiKey: "",
	useAutoTags: false,
	permalinkStyle: PermalinkStyle.NanoId,
};
...
const BUTTON_TEXT_OVERWRITE_FRONTMATTER_ONLY = "Overwrite Front Matter Only";
...
let IS_SETTING_VALIDATION_PASS = false;
...
function generateDeviceIdentifier(): string {
	...
	// 정보를 조합하여 해시 생성
	const identifierString = `${hostname}-${platform}-${arch}`;
...
class OverwriteModal extends Modal {
	...
	onOpen() {
		...
		this.createStyledButton(buttonContainer, BUTTON_TEXT_OVERWRITE_FRONTMATTER_ONLY, false, () => {
			this.resolvePromise(OverwriteChoice.OverwriteFrontMatterOnly);
			this.close();
		});
		...
		try {
			...
			if (isFileExists) {
				...
				if (choice === OverwriteChoice.OverwriteFrontMatterAndContent) {
					try {
						...
						autoGeneratedTags = await this.openAITagExtractor.extractTags(this.contentBody);
						...
						targetFilePath = path.join(targetDir, this.addDateToTitle(sentinizedFileName));
					} catch (error) {
				...
				if (choice === OverwriteChoice.OverwriteFrontMatterOnly) {
					try {
						...
						autoGeneratedTags = this.frontMatter["tags"];

						updatedContentBody = this.contentBody;
			...
			try {
				...
				switch (this.settings.permalinkStyle) {
					case PermalinkStyle.NanoId:
						this.frontMatter["permalink"] = `/${this.frontMatter["nanoId"]}/`;
						break;
					case PermalinkStyle.FileNameUrlEncoded:
						this.frontMatter["permalink"] = `/${encodeURIComponent(sentinizedFileName)}/`;
						break;
				...
				if (!targetFilePath) throw new Error("targetFilePath is undefined");
				if (!updatedContentBody) throw new Error("updatedContentBody is undefined");
...
class JekyllExportSettingTab extends PluginSettingTab {
	...
	display(): void {
		...
		new Setting(renameSection)
			.setName("Permalink")
			.setDesc("Style used to add a permalink to the front matter during export")
			.addDropdown((dropdown) => {
				dropdown.selectEl.addClass("folder-select");
...
export default class JekyllExportPlugin extends Plugin {
	...
	private deviceId = generateDeviceIdentifier();
	async onload() {
		try {
	...
	async validateSettings(settings: JekyllExportSettings): Promise<boolean> {
		const notices: string[] = [];

		if (settings.activeTargetFolder && !fsSync.existsSync(settings.activeTargetFolder)) {
			notices.push("활성 타켓 폴더가 존재하지 않습니다.");
		}

		if (notices.length > 0) {
			new Notice(notices.join("\n"));
			return false;
		}
	...
	async loadSettings(): Promise<void> {
		try {
			...
			const deviceSettings = data[this.deviceId] || {};
			...
			// 설정 검증
			IS_SETTING_VALIDATION_PASS = await this.validateSettings(this.settings);
		} catch (error) {
